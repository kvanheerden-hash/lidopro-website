...
export function ServerManagementTable({
      ...
      case "paused":
        return (
          <div className="px-3 py-1.5 rounded-lg bg-yellow-500/10 border border-yellow-500/30 flex items-center justify-center">
            <span className="text-yellow-600 text-sm font-medium">Restocking</span>
          </div>
        );
      case "inactive":
        return (
  ...
  return (
    <div className={`w-full max-w-7xl mx-auto p-6 ${className}`}>
      <div className="relative border border-slate-200 rounded-2xl p-6 bg-white shadow-sm">
        {/* Header */}
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <div className="text-sm text-slate-500">
              {servers.filter(s => s.status === "active").length} In Stock • {servers.filter(s => s.status === "inactive").length} Restricted
            </div>
          </div>
        </div>
        ...
        >
          ...
          {servers.map((server) => (
            ...
            >
              <motion.div
                className="relative bg-slate-50 border border-slate-200 rounded-xl p-4 overflow-hidden"
                whileHover={{
                  y: -1,
              ...
              >
                ...
                <div className="relative grid grid-cols-12 gap-4 items-center">
                  ...
                  {/* Service Location */}
                  <div className="col-span-2 flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full overflow-hidden border border-slate-200 flex items-center justify-center">
        ...
        <AnimatePresence>
          {selectedServer && (
            ...
            >
              {/* Header with Actions */}
              <div className="relative bg-slate-50 p-6 border-b border-slate-200 flex items-center justify-between">
                <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                  <motion.button
                    className="w-10 h-10 bg-white hover:bg-slate-50 rounded-full flex items-center justify-center border border-slate-200 shadow-sm ml-2"
              ...
              <div className="flex-1 p-6 space-y-4 overflow-y-auto">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div className="bg-slate-50 rounded-lg p-3 border border-slate-200">
                    ...
                    <div className="text-sm font-mono font-bold mt-1 text-slate-900">
                      {selectedServer.ip}
                    </div>
                  </div>
                ...
                <div className="bg-slate-50 rounded-lg p-4 border border-slate-200">
                  <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3 block">
                    Product Potency Analysis
                  </label>
                  {getCPUBars(selectedServer.cpuPercentage, selectedServer.status)}
                </div>

                <div className="bg-slate-900 rounded-lg p-4 text-slate-400">
                  <label className="text-xs font-bold text-brand-400 uppercase tracking-widest mb-2 block">
                    Clinical Audit Trail
                  </label>
                  <div className="font-mono text-xs space-y-1 max-h-24 overflow-y-auto">
                    <div className="text-green-400">[SYST] Batch verified successfully</div>
                    <div className="text-blue-400">[CHEM] Formulation stability confirmed</div>
                    <div className="text-yellow-400">[MON] Potency check: {selectedServer.cpuPercentage}%</div>
                    <div className="text-slate-500">[AUTH] Logged access by Clinical Admin</div>
                  </div>
                </div>
              </div>
            </motion.div>
